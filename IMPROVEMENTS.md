...
> ملاحظة: القسم التالي يوثّق منذ الإصدار v2.0.0 حالة المشروع بعد إعادة الهيكلة (انظر [`CHANGELOG.md`](./CHANGELOG.md)). القسمان أدناه (كتالوج الأفكار وخطة إعادة الهيكلة) يبقيان كسجل تاريخي للقرارات المتخذة.

- كود مُقسَّم إلى وحدات ES Modules (`js/`)، مع أنواع عبر JSDoc + `tsc --noEmit`.
- صفر اعتمادات وقت تشغيل؛ يتطلب خادماً محلياً للتطوير بسبب `fetch` على ملفات الترجمة (انظر قسم "التطوير المحلي" في README).
- يدعم العربية والإنجليزية مع RTL/LTR، ووضعاً ليلياً تلقائياً/يدوياً.
- ميزات حالية: إحداثيات حية، حفظ نقاط، قياس مسافة، تحليل ألوان (RGB/HEX)، تصدير CSV، تكبير/تصغير 50%-300%، شبكة مساعدة، تراجع/إعادة، اختصارات لوحة المفاتيح.

### الميزات الموعودة في README (لم تُنفّذ بعد)

حفظ المشروع، رسم خطوط وأشكال، قياس الزوايا، دعم PDF، الطباعة.
